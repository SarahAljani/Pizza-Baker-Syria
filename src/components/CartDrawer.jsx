  ...
  const handleCheckout = () => {
    ...
    if (language === 'ar') {
      ...
      cart.forEach((item, index) => {
        const itemSize = item.size === 'small' ? 'صغير' : item.size === 'medium' ? 'وسط' : 'كبير';
        messageText += `${index + 1}. *${item.name}* (حجم: ${itemSize})\n`;
      ...
      cart.forEach((item, index) => {
        messageText += `${index + 1}. *${item.name}* (${item.size.toUpperCase()})\n`;
                ...
                <>
                  {cart.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center gap-4 py-20">
                      ...
                      <h4 className="font-serif text-lg text-text-primary font-medium">{t('basketIsEmpty')}</h4>
                          ...
                          <div className="space-y-1.5 flex-1 pr-4 pl-4 text-start">
                            ...
                            {item.customToppings && item.customToppings.length > 0 && (
                              <div className="bg-bg-primary border border-border-primary p-2 rounded text-[10px] text-text-secondary font-mono space-y-0.5 transition-colors">
                                <span className="text-text-tertiary block uppercase font-bold mb-1">{t('composition')}:</span>
                            ...
                            <span className="font-mono text-xs text-brand-gold font-bold block">
                              {item.price} {isRtl ? 'ل.س' : 'SYP'} {isRtl ? 'للوحدة' : 'each'}
                            </span>
                          </div>
                  ...
                  <div className="bg-bg-primary border border-border-primary p-5 w-full font-mono text-xs text-start text-text-secondary space-y-2.5 transition-colors">
                    <div className="text-center border-b border-border-primary pb-3 mb-2">
                      <span className="font-serif text-sm text-text-primary font-bold block uppercase tracking-wider">
                        {isRtl ? 'فاتورة خباز البيتزا سوريا' : 'PIZZA BAKER SYRIA RECEIPT'}
                      </span>
                    ...
                    {cart.map((item) => (
                      <div key={`${item.id}-${item.size}`} className="flex justify-between">
                        <span>{item.quantity}x {item.name} ({t(item.size).toUpperCase()})</span>
                        <span className="text-text-primary">{item.price * item.quantity} {isRtl ? 'ل.س' : 'SYP'}</span>
                      </div>
                    ))}
                    ...
                    <div className="border-t border-border-primary pt-3 mt-3 space-y-2">
                      <div className="flex justify-between text-[11px]">
                        ...
                        <span>{subtotal} {isRtl ? 'ل.س' : 'SYP'}</span>
                      </div>
                      <div className="flex justify-between text-[11px]">
                        <span>{t('delivery')}</span>
                        <span>{deliveryFee === 0 ? t('free') : `${deliveryFee} ${isRtl ? 'ل.س' : 'SYP'}`}</span>
                      </div>
                      <div className="flex justify-between text-text-primary font-bold text-sm border-t border-dashed border-border-primary pt-2">
                        ...
                        <span className="text-brand-gold">{total} {isRtl ? 'ل.س' : 'SYP'}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-bg-primary border-t border-border-primary p-5 space-y-4 transition-colors">
                <div className="space-y-2 font-mono text-xs text-text-secondary text-start">
                  <div className="flex justify-between">
                    <span>{t('subtotal')}</span>
                    <span className="text-text-primary">{subtotal} {isRtl ? 'ل.س' : 'SYP'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('delivery')}</span>
                  ...
                  {subtotal < 3000 && (
                    <span className={`text-[9px] font-sans text-brand-gold block ${isRtl ? 'text-left' : 'text-right'}`}>
                      {language === 'ar' ? (
                        <span>أضف بقيمة <strong className="text-text-primary font-semibold">{3000 - subtotal} ل.س</strong> إضافية للتوصيل المجاني!</span>
                      ) : (
                        <span>Add <strong className="text-white font-semibold">{3000 - subtotal} SYP</strong> more for FREE delivery!</span>
                      )}
                    </span>
                  )}
