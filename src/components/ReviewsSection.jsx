...
import { Star, MessageSquare, Check, User, Quote, Send } from 'lucide-react';
...
export default function ReviewsSection() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setReviews(INITIAL_REVIEWS);
      localStorage.setItem('pizzabaker_reviews_v3', JSON.stringify(INITIAL_REVIEWS));
    }
  }, []);
  ...
  const handleSubmitReview = (e) => {
    ...
    const addedReview = {
      ...
      date: new Date().toLocaleDateString(language === 'ar' ? 'ar-SY' : 'en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
      }),
    ...
    localStorage.setItem('pizzabaker_reviews_v3', JSON.stringify(updatedReviews));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-7 space-y-6 max-h-[580px] overflow-y-auto pr-2 custom-scroll">
            ...
            <AnimatePresence initial={false}>
              {reviews.map((review) => (
                ...
                >
                  <Quote className={`absolute ${isRtl ? 'left-6' : 'right-6'} bottom-6 w-12 h-12 text-white/[0.02] pointer-events-none group-hover:text-white/[0.04] transition-colors`} />
                      ...
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          ...
                          className={`w-3.5 h-3.5 ${
                            i < review.rating ? 'text-brand-gold fill-brand-gold' : 'text-text-tertiary'
                          }`}
                        />
                      ))}
                    </div>
          ...
          <div className="lg:col-span-5 bg-bg-secondary border border-border-primary p-6 sm:p-8 relative transition-colors duration-300">
            <div className={`flex items-center gap-2 mb-6 border-b border-border-primary pb-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
