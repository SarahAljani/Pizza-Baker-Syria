...
import { Calendar, Users, Clock, Mail, User, Ticket, ChefHat } from 'lucide-react';
...
export default function ReservationSection() {
  ...
  const handleSubmit = (e) => {
    ...
    const subject = encodeURIComponent(`Table Reservation at Pizza Baker Syria - ${formData.name}`);
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <User className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 w-4 h-4 text-brand-gold`} />
                      <input
                        type="text"
                        required
                        id="input-res-name"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Mail className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 w-4 h-4 text-brand-gold`} />
                      <input
                        type="email"
                        required
                        id="input-res-email"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Calendar className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 w-4 h-4 text-brand-gold`} />
                      <input
                        type="date"
                        required
                        id="input-res-date"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Clock className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 w-4 h-4 text-brand-gold`} />
                      <input
                        type="time"
                        required
                        id="input-res-time"
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-mono text-xs font-bold text-brand-gold">
                        {formData.guests} {language === 'ar' ? t('guestUnit') : (formData.guests === 1 ? 'PERSON' : 'PEOPLE')}
                      </span>
                    </div>

                    <div className="relative flex items-center h-[50px] bg-bg-primary border border-border-primary px-4">
                      <Users className={`w-4 h-4 text-brand-gold ${isRtl ? 'ml-4' : 'mr-4'} flex-shrink-0`} />
                      <input
                        type="range"
                        min="1"
                        max="12"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isRtl ? 'مثال: طاولة قرب الفرن الحجري، إطلالة على الشارع، ملاحظات الحساسية...' : 'E.g. Table near the woodfired brick oven, window view, allergy notes...'}
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      />
                      <span>{isRtl ? 'جاري تحضير بريد الحجز...' : 'PREPARING RESERVATION EMAIL...'}</span>
                    </>
                  ) : (
                    <>
                      <ChefHat className="w-4 h-4" />
                      <span>{isRtl ? 'تأكيد الحجز عبر البريد الإلكتروني' : 'SECURE RESERVATION BY EMAIL'}</span>
                    </>
                  )}
                </button>
              </motion.form>
                ...
                <div className="max-w-md mx-auto bg-bg-primary border-2 border-brand-gold p-8 relative shadow-2xl overflow-hidden">
                  ...
                  <div className="space-y-4 border-t border-b border-border-primary py-5 text-left mb-6 font-mono text-xs text-text-primary">
                    <div className="flex justify-between">
                      <span className="text-text-secondary uppercase">{isRtl ? 'رقم المرجع' : 'REFERENCE ID'}</span>
                      <span className="text-text-primary font-bold">{ticket.id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary uppercase">{isRtl ? 'الاسم الكامل' : 'FULL NAME'}</span>
                      <span className="text-text-primary font-bold">{ticket.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary uppercase">{isRtl ? 'عدد الأشخاص' : 'PARTY SIZE'}</span>
                      <span className="text-text-primary font-bold text-brand-gold">
                        {ticket.guests} {language === 'ar' ? t('guestUnit') : 'PEOPLE'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary uppercase">{isRtl ? 'التاريخ والوقت' : 'DATE & TIME'}</span>
                      <span className="text-text-primary font-bold">{ticket.date} @ {ticket.time}</span>
                    </div>
                    {ticket.specialRequests && (
                      <div className="border-t border-border-primary pt-3 mt-3">
                        <span className="text-text-secondary uppercase block mb-1">{isRtl ? 'ملاحظات خاصة' : 'NOTES'}</span>
                  ...
                  <div className="text-text-secondary font-sans text-xs italic font-light mb-6">
                    {isRtl ? (
                      <span>لقد قمنا بفتح تطبيق البريد الإلكتروني الخاص بك لإرسال الحجز إلى <strong className="text-text-primary">adnan195662@gmail.com</strong> و <strong className="text-text-primary">info@pizzabakersyria.com</strong>.</span>
                    ) : (
                      <span>We have launched your email client to send this booking to <strong className="text-text-primary">adnan195662@gmail.com</strong> and <strong className="text-text-primary">info@pizzabakersyria.com</strong>.</span>
                    )}
                  </div>
