...
export default function PizzaMenu({ onAddToCart, cart }) {
  ...
  const filteredMenu = INITIAL_MENU
    ...
    .filter((item) => {
      ...
      return numStr.includes(q) || nativeName.includes(q) || translatedName.includes(q) || translatedDesc.includes(q);
    });
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.02] dark:opacity-[0.03] text-text-primary">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <pattern id="pizza-pattern-grid" width="120" height="120" patternUnits="userSpaceOnUse">
            <path
              ...
              strokeWidth="1.5"
            />
            <circle cx="25" cy="85" r="8" fill="none" stroke="currentColor" strokeWidth="1.5" />
            <circle cx="75" cy="35" r="12" fill="none" stroke="currentColor" strokeWidth="1.5" />
          </pattern>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <div className="mt-8 inline-grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-8 bg-bg-secondary border border-border-primary p-4 sm:p-6 text-left max-w-2xl mx-auto transition-all">
            ...
            <div className={`flex items-start gap-2.5 border-t sm:border-t-0 border-border-primary pt-3 sm:pt-0 ${isRtl ? 'sm:border-r sm:pr-6 sm:border-l-0' : 'sm:border-l sm:pl-6'}`}>
              ...
              <div>
                ...
                <span className="block font-serif text-xs text-text-primary italic">
                  {isRtl ? 'مثالية لشخص إلى شخصين' : 'Perfect for 1 to 2 Persons'}
                </span>
              </div>
            </div>
            <div className={`flex items-start gap-2.5 border-t sm:border-t-0 border-border-primary pt-3 sm:pt-0 ${isRtl ? 'sm:border-r sm:pr-6 sm:border-l-0' : 'sm:border-l sm:pl-6'}`}>
              ...
              <div>
                ...
                <span className="block font-serif text-xs text-text-primary italic">
                  {isRtl ? 'مثالية لشخصين إلى ٣ أشخاص' : 'Ideal for 2 to 3 Persons'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-md mx-auto mb-10 relative">
          <Search className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 w-4.5 h-4.5 text-brand-gold`} />
          <input
            type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence mode="popLayout">
            {filteredMenu.map((pizza) => {
              ...
              const currentPrice = currentSize === 'small'
                ? pizza.prices.small
                : currentSize === 'medium'
                ? pizza.prices.medium
                : pizza.prices.large;
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute top-3 ${isRtl ? 'right-3' : 'left-3'} bg-brand-burgundy text-white font-mono text-[9px] font-black px-2 py-0.5 tracking-wider uppercase z-20 shadow-md`}>
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-6">
                    ...
                    <div className="space-y-3 pt-3 border-t border-border-primary">
                      ...
                      <div className="grid grid-cols-3 gap-1">
                        <button
                          ...
                          className={`py-1.5 text-[10px] font-mono tracking-wider transition-all cursor-pointer ${
                            currentSize === 'small'
                      ...
                      <div className="flex items-center gap-1.5 text-[9px] font-mono text-text-secondary uppercase">
                        {currentSize === 'small' ? (
                          <>
                            ...
                            <span>{isRtl ? 'تكفي شخص واحد' : 'SERVES 1 PERSON'}</span>
                          </>
                        ) : currentSize === 'medium' ? (
                          <>
                            <Users className="w-3 h-3 text-brand-soft-yellow" />
                            <span>{isRtl ? 'تكفي شخص إلى شخصين' : 'SERVES 1 - 2 PERSONS'}</span>
                          </>
                        ) : (
                          <>
                            <Users className="w-3 h-3 text-brand-brown" />
                            <span>{isRtl ? 'تكفي شخصين إلى ٣ أشخاص' : 'SERVES 2 - 3 PERSONS'}</span>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-border-primary pt-4">
                      <div className="text-left leading-tight">
                        <span className="text-[8px] font-mono text-text-secondary block uppercase">{isRtl ? 'السعر' : 'PRICE'}</span>
                        <span className="text-lg font-mono font-bold text-text-primary">
                          {currentPrice} <span className="text-[10px] text-brand-gold">{isRtl ? 'ل.س' : 'SYP'}</span>
                        </span>
                      </div>
                        ...
                        >
                          {quantityInCart > 0 ? (
                            <>
                              ...
                              <span>{isRtl ? `في السلة (${quantityInCart})` : `IN CART (${quantityInCart})`}</span>
                            </>
                          ) : (
                            <>
